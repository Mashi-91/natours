...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.updatePasswordAfter(decoded.iat)) {
    return next(
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    next(
      new AppError(
        'There was an error sending the email. Please try again. Try again later.',
        500,
      ),
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const hastToken = crypto
    .createHash('sha256')
    .update(req.params.token)
    .digest('hex');

  const user = await User.findOne({
    passwordResetToken: hastToken,
